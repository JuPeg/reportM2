...
Overall, these algorithms' computational complexity is on the order of _O_(NM), where N indicates the number of sequences in the input and M is the number of representative sequences. Therefore, their resources consumption is quite hard to predict since the data redundancy may be unknown until the algorithm is run. [^Cai]
